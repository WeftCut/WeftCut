...
export const MCP_TOOL_DEFS: ReadonlyArray<McpToolDef> = [
  ...
  { name: 'groups_add_members', exec: 'table',
    description: "Move existing layers INTO the composition a Group layer already shows — the third and last op that crosses compositions, beside `groups_create` and `groups_ungroup`. `layer_ids` are the layers to move, at least one and all in ONE composition; `group_layer_id` is the Group clip in that SAME composition, and its `params.composition` is the destination. Members land at `t_start_us − group.t_start_us + group.src_in_us`, both endpoints re-snapped on the layer's own lattice, so they keep the screen position they had: a clip visible under the Group clip stays where it looked, and a member outside the Group clip's window arrives outside it and shows as overhang. Destination lanes map per SOURCE TRACK, bottom-up onto the destination's existing lanes (A roll, B roll, then whatever else it has) and spawning a lane past the end — a whole source track's members travel together onto ONE destination lane, which is what keeps a transition between two of them alive, and the block bounces to the nearest free lane (else a fresh one) when the preferred lane is locked or already occupied at those times. Links fully inside the set move with their ids; a straddling link loses its inside members and dissolves below 2. Transitions between two moved members move; a straddling one is dropped and logged. Markers stay in the source composition. BOTH compositions autofit afterwards and NO Group layer is retrimmed: a destination that grew changes the overhang of every Group clip showing it, this one included — widen a window yourself with `trim_layer` if you want it to follow. Refuses whole, before any write, so a refused call changes nothing: a set spanning two compositions or a Group clip outside the members' composition (`CrossCompositionSet`), a `group_layer_id` that is not a Group layer (`WrongLayerKind`), a locked member (`GroupLockedMember`) or locked source track (`TrackLocked`), a member whose own composition already reaches the destination — the destination itself included, which is the move-a-Group-into-itself case (`ValidationFailed` / `CompositionCycle` with the loop spelled out), and a member that would land before composition time 0 (`InvalidArgument` on `layer_ids`, naming the earliest source time that fits). One undo puts every member back on its lane, at its time, with its links and transitions.",
    inputSchema: { type: 'object', properties: {
