...
import { applyLinksCreate, applyLinksDissolve, applyLinksAddMembers, applyLinksRemoveMembers } from './links'
...
describe('link mutations', () => {
  it('create: rejects < 2 unique members', () => {
    ...
    expectCmd(() => applyLinksCreate(p, seededGen(), ['a', 'a'], false), 'LinkCreateNeedsTwoLayers')
  })
  it('create: rejects a missing member', () => {
    const p = withLayers(['a', 'b'])
    expectCmd(() => applyLinksCreate(p, seededGen(), ['a', 'ghost'], false), 'LayerNotFound')
  })
  it('create: makes a link with sorted members; label omitted when null', () => {
    const p = withLayers(['a', 'b'])
    const gen = seededGen()
    const gid = applyLinksCreate(p, gen, ['b', 'a'], false)
  ...
  it('create: rejects an already-linked layer unless reassign', () => {
    ...
    expectCmd(() => applyLinksCreate(p, seededGen(), ['b', 'c'], false), 'LayerAlreadyLinked')
    // reassign moves 'b' to the new link; old link drops to 1 member → auto-dissolves
    applyLinksCreate(p, seededGen(), ['b', 'c'], true)
    expect(root(p).links.length).toBe(1)
  ...
  it('removeMembers: removes, auto-dissolves below 2, errors on non-member', () => {
    const p = withLayers(['a', 'b', 'c'])
    const gid = applyLinksCreate(p, seededGen(), ['a', 'b', 'c'], false)
...
describe('link mutations inside a Group', () => {
  ...
  it("links_create lands in the members' composition; a mixed set is CrossCompositionSet; dissolve finds the link by id", () => {
    ...
    expectCmd(() => applyLinksCreate(p, idGen, [innerId, refLayerId], false), 'CrossCompositionSet')
    const gid = applyLinksCreate(p, idGen, [innerId, second], false)
    ...
    const rootLink = applyLinksCreate(p, idGen, [refLayerId, applyAddLayer(p, idGen, root(p).tracks[1].id, colorParams(BLACK, 1, 1), 0, 500_000)], false)
