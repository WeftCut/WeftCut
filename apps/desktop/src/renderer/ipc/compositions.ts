...
/// names a composition the summary no longer carries. The anchor store may point
/// at a missing id for the span of one `apply` (it falls back on the same tick
/// — see `reconcileCompositionAnchors`); a consumer rendering in that gap gets a
...
/// The root — what export renders, whichever composition the editor has focused
/// (`state/compositionAnchorStore.ts` says why). Throws rather than returning a
