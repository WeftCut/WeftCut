...
export function PositionConversionFields({ position, layerId, durationUs, busy, onApply }: {
    ...
    const field = (label: string, value: number, set: (n: number) => void, min: number, step = 1) => <label className="flex items-center justify-between gap-2 text-xs">
    {label}<input className="w-20 rounded border bg-transparent px-2 py-1" type="number" min={min} step={step} value={value} disabled={busy} onChange={e => { clear(); set(Number(e.target.value)); }}/>
  </label>;
    ...
    {position.mode === 'Path' && field(t('motion_path.every_frames'), everyFrames, setEvery, 1)}
    ...
    {result && result.nodeCount > 0 && <p className="text-xs">{t('motion_path.nodes_used', { count: result.nodeCount })}</p>}
    {result && !result.withinTolerance && <p role="alert" className="text-xs text-destructive">{t(`motion_path.${result.limit}`)}</p>}
    {error && <p role="alert" className="text-xs text-destructive">{error}</p>}
    <div className="flex flex-wrap gap-2">
      <Button size="sm" variant="outline" disabled={busy} onClick={preview}>{t('motion_path.preview')}</Button>
      <Button size="sm" disabled={busy || !result?.withinTolerance} onClick={async () => { if (result?.withinTolerance && await onApply(result.position))
