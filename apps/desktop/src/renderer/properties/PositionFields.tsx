...
export function PositionFields(props: {
    ...
    const insert = async () => {
        if (!path || selected < 0 || selected >= path.path.nodes.length - 1) return;
    ...
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
      ...
      {path && edit.layerId === layer.id && <>
        <label className="flex items-center gap-2 text-xs">{t('motion_path.node_mode')}
          <select aria-label={t('motion_path.node_mode')} className="rounded border bg-background px-2 py-1" disabled={busy || selected < 0} value={selected < 0 ? '' : path.path.nodes[selected]!.tangentMode} onChange={e => void change({ ...path, path: setPathNodeMode(path.path, selected, e.target.value as PathNode['tangentMode']) }, true)}>
            {selected < 0 && <option value="">{t('motion_path.select_node')}</option>}
            <option value="Corner">{t('motion_path.mode_corner')}</option><option value="Smooth">{t('motion_path.mode_smooth')}</option><option value="Auto">{t('motion_path.mode_auto')}</option>
          </select>
        </label>
        <Button size="sm" variant="outline" disabled={busy || selected < 0 || selected >= path.path.nodes.length - 1 || path.path.nodes.length >= 128} onClick={() => void insert()}>{t('motion_path.insert')}</Button>
        <Button size="sm" variant="outline" disabled={busy || path.path.nodes.length >= 128} onClick={() => { const last = path.path.nodes.at(-1)!; updateNodes([...path.path.nodes, { ...last, tangentMode: 'Corner', id: crypto.randomUUID(), point: { x: last.point.x + 100, y: last.point.y }, inHandle: { x: 0, y: 0 }, outHandle: { x: 0, y: 0 }, segment: 'Line' }]); }}>{t('motion_path.add')}</Button>
    ...
    {path && edit.layerId === layer.id && <p className="text-xs text-muted-foreground">{t('motion_path.insert_hint')}</p>}
