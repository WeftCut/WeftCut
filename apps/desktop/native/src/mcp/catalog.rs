...
tool_table! {
    ...
    "import_media" => ("Import a media file from an absolute path. Hashes the file (blake3) and probes \
                          ...
                          a subtitle document (.srt/.vtt/.ass) is laid onto a caption track instead and returns `{ caption_track_id, cues }`.", tools::ImportMediaArgs, tools::import_media),
    #[cfg(feature = "speech")]
