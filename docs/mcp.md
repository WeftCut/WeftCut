...
- `get_param_track { layer_id, param_key }` → `{ mode, value }` (Static) or `{ mode, extrapolate: { before, after }, keyframes: [{ id, t_us, t_local_us, value, in, out, continuity, segment, preset_id? }] }` (Keyframed). Read this to discover keyframe ids before editing. `in` / `out` are the key's arriving / leaving tangents `{ x, y, mode }` and `segment` the class of the segment leaving it (`Spline | Hold | Linear | Elastic | Bounce`; only Spline reads the tangents — [`data-model.md` § Animated values](data-model.md#animated-values)). `preset_id` names the canonical easing preset the segment leaving the key (this key's `segment` + `out`, the next key's `in`) exactly matches (exact-float reverse lookup — presets bake to tangents, the name is recovered on read); a hand-tuned curve and the last key omit the field.
...
- `set_keyframe_easing { layer_id, param_key, keyframe_id, interp }` — writes the segment leaving the key (this key's `segment` and `out`, the next key's `in`, both sides Free). `interp` is one of:
...
- `smooth_keyframes { layer_id, param_key, keyframe_id? }` — set Auto tangents (clamped monotone, re-solved on every write) with Smooth continuity on one key, or on every key when `keyframe_id` is omitted; the segments on either side become Spline.
- `clear_keyframes { layer_id, param_key, value? }` — collapse to Static (defaults to the first keyframe's value).
- `set_param_track { layer_id, param_key, track }` — low-level: replace the whole `AnimTrack<f64>` in the `get_param_track` record shape (keyframe `t_us` timeline-absolute; `extrapolate` defaults to Hold / Hold when omitted).
