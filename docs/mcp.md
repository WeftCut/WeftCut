...
| `media://{id}/description` | cached scene descriptions for the resolver's default backend + sampling + the app's UI language (`{ covered_ranges, segments }`); `404` until `describe_clip` has populated it at that key (unlike the always-computable resources above) — the refusal names the language, since a language switch reads as "not described" until the shots are described again |
