...
- `add_effect { layer_id, kind }` → `{ effect_id, layer_id, kind, enabled, index, params }`. Append an effect to the end of the chain (applied last). Creates the effect with no params set; use `update_effect` to set a static value or `set_keyframe` to keyframe a param.
