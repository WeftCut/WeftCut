...
| Analysis surfaces (Shots Panel analyze and apply, the silence dialog, the describe gestures) | `Project` | `Info`/`Error` | `source = User`. Started → Ok/Err under one `op_id`, always the pair even when an apply settles inside 250 ms: the Started row is the only statement of how much of the review went out (`log.shots_apply_*_started` carries the cut count and, for a discard, the discarded count), and the terminal row says what came back — segments, markers or survivors (`log.shots_apply_*_done`), candidates found (`log.shots_analyze_done`), spans measured (`log.shots_stats_done`), silence markers added (`log.mark_silences_done`) or silent stretches cut out with the time that went with them (`log.remove_silences_started` / `log.remove_silences_done`), described spans with the engine and model that answered (`log.describe_done`). The measure pass carries the same span count on both rows (`log.shots_stats_started`), because that count is what it costs — three ffmpeg extracts per span nothing had measured. A structured refusal closes the op under its own curated key; only a prose failure uses `log.shots_analyze_failed` / `log.shots_stats_failed` / `log.shots_apply_failed`, and the silence dialog's and the describe gestures' prose failures are `logMutationFailure`'s generic row under the same `op_id`. A failure also stays inline wherever the gesture has a slot — the Panel's own describe slot, the silence dialog — and the describe command has none, so its one recoverable refusal (no engine configured) opens Settings → Video understanding instead of reporting twice. |
