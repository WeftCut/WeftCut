...
| Direct commits (inspector fields and keyframe/effect commits, drag commits — timeline and on-canvas gizmo, drops onto the timeline, timeline context menus, mixer, caption restyle, proxy mode) | `Project` | `Error` (`Debug` for no-op refusals) | One entry per refused mutation via `renderer/errors/tryMutate.ts`; `details` carries the structured `CommandError`. Components with their own inline error slot (media-removal dialog, effects section, project settings) keep it and render the same refusal copy there instead. |
