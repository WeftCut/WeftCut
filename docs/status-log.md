...
| Direct commits (inspector fields and keyframe/effect commits, drag commits — timeline and on-canvas gizmo, drops onto the timeline, timeline context menus, mixer, caption restyle, proxy mode) | `Project` | `Error` (`Debug` for no-op refusals) | One entry per refused mutation via `renderer/errors/tryMutate.ts`; `details` carries the structured `CommandError`. Components with their own inline error slot (media-removal dialog, effects section, project settings, the voiceover dialog) keep it and render the same refusal copy there instead. A refusal stated in prose rather than as a structured `CommandError` has Electron's `Error invoking remote method 'backend:invoke': ` wrapper stripped before it is shown or logged, so the sentence that names the remedy ("configure one in Settings → Transcription") is what the user reads. |
| Speech runs (the transcribe command, the voiceover dialog) | `Project` | `Info`/`Error` | `source = User`. Started → Ok under one `op_id` — both are multi-second network calls, and without the Started row the status badge has nothing to spin on. Keys `log.auto_caption_started` (one clip, named) or `log.auto_caption_started_many` (several, counted) / `log.auto_caption_done` (cue count over every transcript + the engine, or engines, that served) and `log.voiceover_started` / `log.voiceover_done` / `log.voiceover_done_cached` — two terminal keys because the difference is what the run cost: a cached hit billed nothing. Rows carry the script's LENGTH, the voice and the speed, never the script; a test asserts no row or `details` contains it. Failures go through `logMutationFailure` (the direct-commits row above) handed the run's `op_id`, so the failure row closes the op as `Err` — the running badge clears only on a terminal row under the same id — and, for voiceover, also stay inline in the dialog. The transcribe run's READ step is the exception: a clip that fails is logged as `log.auto_caption_failed`, which names the clip and carries the tool's own sentence, because a multi-clip run's user has to know which clip to fix; the run stops there and still applies the transcripts before it, so a `log.auto_caption_done` row with no `op_state` precedes the `Err` row that closes the op. Only the write step (`apply_subtitles`) uses the generic row. The transcribe command has no dialog, so its refusal is read in the status bar, and one that names Settings → Transcription also opens that pane. |
| Analysis surfaces (Shots Panel analyze and apply, the Pauses section, the describe gestures) | `Project` | `Info`/`Error` | `source = User`. Started → Ok/Err under one `op_id`, always the pair even when an apply settles inside 250 ms: the Started row is the only statement of how much of the review went out (`log.shots_apply_*_started` carries the cut count and, for a discard, the discarded count), and the terminal row says what came back — segments, markers or survivors (`log.shots_apply_*_done`), candidates found (`log.shots_analyze_done`), spans measured (`log.shots_stats_done`), pause markers added (`log.mark_pauses_done`) or pauses cut out with the time that went with them (`log.remove_pauses_started` / `log.remove_pauses_done`), described spans with the engine and model that answered (`log.describe_done`). The measure pass carries the same span count on both rows (`log.shots_stats_started`), because that count is what it costs — three ffmpeg extracts per span nothing had measured. A structured refusal closes the op under its own curated key; only a prose failure uses `log.shots_analyze_failed` / `log.shots_stats_failed` / `log.shots_apply_failed`, and the Pauses section's and the describe gestures' prose failures are `logMutationFailure`'s generic row under the same `op_id`. A failure also stays inline wherever the gesture has a slot — the Panel's own describe slot, the Pauses section — and the describe command has none, so its one recoverable refusal (no engine configured) opens Settings → Video understanding instead of reporting twice. |
