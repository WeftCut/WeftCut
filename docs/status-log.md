...
| Analysis surfaces (Shots Panel analyze and apply, the silence dialog) | `Project` | `Info`/`Error` | `source = User`. Started → Ok/Err under one `op_id`, always the pair even when an apply settles inside 250 ms: the Started row is the only statement of how much of the review went out (`log.shots_apply_*_started` carries the cut count and, for a discard, the discarded count), and the terminal row says what came back — segments, markers or survivors (`log.shots_apply_*_done`), candidates found (`log.shots_analyze_done`), silence markers added (`log.mark_silences_done`). A structured refusal closes the op under its own curated key; only a prose failure uses `log.shots_analyze_failed` / `log.shots_apply_failed`, and the silence dialog's prose failure is `logMutationFailure`'s generic row under the same `op_id`. Every failure also stays inline in the Panel or dialog. |
